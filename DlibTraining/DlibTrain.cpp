...
std::vector<ImageF> load_mnist_data(const std::string & filename) {
    ...
    int magic = data_read(file);
    int img_idx = data_read(file);
    int rows = data_read(file);
    int cols = data_read(file);

    std::vector<ImageF> images(img_idx);
    for (int ii = 0; ii < img_idx; ii++) {
...
std::vector<double> load_mnist_labels(const std::string & filename) {
    ...
    for (uint32_t ii = 0; ii < labels_idx; ii++) {
        ...
        labels[ii] = static_cast<unsigned long>(label);
    }
